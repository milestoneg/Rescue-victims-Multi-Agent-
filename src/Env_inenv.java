...
public class Env extends Environment {
		...
		for (int widthIndex = 1; widthIndex < model.getWidth() - 1; widthIndex++) {
			for (int heightIndex = 1; heightIndex < model.getHeight() - 1; heightIndex++) {
				if (model.isFreeOfObstacle(widthIndex, heightIndex)) {
					...
					possibilities.put(new Location(widthIndex, heightIndex), PossibleDirections);
	...
	private void possible_location() {
        ...
        System.out.println("Size of key: "+ possibilities.keySet().size()); 
		
        Iterator<Location> key_iterator = possibilities.keySet().iterator();
		//for (Location key : possibilities.keySet()) {
        while(key_iterator.hasNext()) {
			LinkedList<DirectionVector> delete_index = new LinkedList<DirectionVector>();
			Location key = key_iterator.next();
			for (int index = 0; index < possibilities.get(key).size(); index++) {
				
				
				Boolean[] map_information = new Boolean[4];
				DirectionVector direction = possibilities.get(key).get(index);
				...
				if (!Arrays.equals(map_information, scan_information)) {
					// System.out.println(possibilities.get(key).size()+"before");
					delete_index.add(direction);		
					//possibilities.get(key).remove(direction);
					// System.out.println(possibilities.get(key).size()+"after");
				}
			}
			for(DirectionVector delete_Vectors :delete_index) {
				possibilities.get(key).remove(delete_Vectors);
			}
			// if a locations' possible directions equals to 0
			/*if (possibilities.get(key).isEmpty()) {
				possibilities.keySet().remove(key);
			}*/
		}
		// if a locations' possible directions equals to 0
		Iterator<Location> delet_iterator = possibilities.keySet().iterator();

		while  (delet_iterator.hasNext()) {
			LinkedList<DirectionVector> i = possibilities.get(delet_iterator.next());
			if(i.isEmpty()) {
				delet_iterator.remove();
			}
        }
		//Debug
		System.out.println(possibilities.keySet().size() + "size");
...
private void loclization_move_next() {
	...
	if(scan_information[0] == false) {
		...
		for(Location location : possibilities.keySet()) {
			...
			for(DirectionVector directionVector : possibilities.get(location)) {
				...
				try {
				 directions = possibilities.get(newlocation);
	...
	for(Location location : possibilities.keySet()) {
		for(DirectionVector directionVector : possibilities.get(location)) {
			...
			LinkedList<DirectionVector> directions = possibilities.get(newlocation);
			try {
				 directions = possibilities.get(newlocation);
	...
	//debug
	System.out.println("After move size of key:"+possibilities.keySet().size());
	for(Location location : possibilities.keySet()) {
		for(DirectionVector directionVector : possibilities.get(location)) {
